...
- Application-level authorization beyond "is this person in the family
  group" (e.g. per-event permissions) — there's one shared calendar, so
  there's nothing finer-grained to model yet.
